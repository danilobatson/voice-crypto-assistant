...
export function VoiceAPITest() {
	...
	const testVolume = () => {
		speakText(
			'Volume test. If you can hear this, your volume is working correctly.'
		);
	};
	...
	const testAPI = async (query: string, attempt: number = 1) => {
		...
		try {
			...
			console.log(
				`📡 API Response in ${endTime - startTime}ms:`,
				result.status
			);
			...
			if (!result.ok) {
				...
				if (result.status === 429) {
					...
					throw new Error(
						errorData.error || 'Invalid request. Please check your query.'
					);
			...
			if (attempt >= maxRetries) {
				setError(
					`Analysis failed after ${maxRetries} attempts: ${errorMessage}`
				);
			} else {
	...
	return (
		<div className='card'>
			...
			<p>
				Ask about any cryptocurrency using your voice - with edit option for
				tricky names!
			</p>
			...
			<div
				...
				💡 Shortcuts: Spacebar (start/stop voice) • Enter (submit) • Escape
				(cancel)
			</div>
			...
			<div className='settings-panel'>
				...
				<div
					...
					{!isOnline && (
						<span className='status-indicator error'>🌐 Offline</span>
					)}
				</div>
			</div>
			...
			{retryCount > 0 && (
				<div className='status info'>
					...
					<br />
					Having trouble connecting to the analysis service.
				</div>
			)}
			...
			<div style={{ textAlign: 'center', margin: '2rem 0' }}>
				<button
					...
					className={`button ${
						listening || loading || processingQuery ? 'danger' : ''
					}`}
					style={{
						fontSize: '1.2rem',
						padding: '1rem 2rem',
						...
						<div style={{ flex: 1 }}>
							...
							{response && (
								<div
									...
									✅ Analysis complete • Response time: {response.responseTime}
									ms
								</div>
							)}
						</div>
			...
			{listening && !isEditing && (
				<div className='status info listening-status'>
					...
					<br />
					Ask about any cryptocurrency. I'll auto-submit after 4 seconds of
					silence.
					<br />
					<em>Click "Edit Query" below if the text looks wrong!</em>
				</div>
			)}
				...
				<div className='transcript-container'>
					...
					<div
						...
						<div
							...
							{listening
								? 'Auto-submit in 4 seconds of silence...'
			...
			{isEditing && (
				<div
					...
					<div
						...
						💡 Perfect for tricky names like "Axie Infinity", "Chainlink",
						"Uniswap", etc.
					</div>
				</div>
			)}
				...
				<details>
					...
					<div
						...
						<div
							...
							Press Enter to submit • Great for hard-to-pronounce cryptocurrency
							names
						</div>
					</div>
				</details>
							...
							{queryHistory.map((historyQuery, index) => (
								<div
									...
									<button
										...
										disabled={
											loading || listening || processingQuery || isEditing
										}
										className='button'
										style={{
											background: '#007bff',
			...
			{response && (
				<div>
					...
					<div className='status success'>
						...
						<br />
						Cryptocurrency: <strong>{response.symbol}</strong>
						<br />
						Tools Used: {response.toolsUsed} | Data Points:{' '}
						{response.dataPoints}
						...
						{response.crypto_detection?.correction_made && (
							<>
								...
								{response.crypto_detection.original_term} →{' '}
								{response.crypto_detection.detected_crypto}
							</>
						)}
					</div>
					...
					<div
						style={{
							background: '#f8f9fa',
							border:
								'2px solid ' + getRecommendationColor(response.recommendation),
							borderRadius: '8px',
							padding: '1rem',
							margin: '1rem 0',
						}}>
					...
					<div className='voice-response-section'>
							...
							<div className='speaking-indicator'>
								<div
									...
									<div>
										...
										<br />
										Can't hear it? Check your volume or use the controls above.
									</div>
								</div>
							</div>
						)}

						<div className='voice-text'>{response.spokenResponse}</div>
						...
						<div
							...
							💡 This text was read aloud at {speechRate}x speed. Use audio
							controls to replay or pause/resume.
						</div>
					</div>
					...
					<div
						...
						<div
							...
							<div>
								<strong>Price:</strong> {response.key_metrics.price}
							</div>
							<div>
								<strong>Galaxy Score:</strong>{' '}
								{response.key_metrics.galaxy_score}
							</div>
							<div>
								<strong>Alt Rank:</strong> {response.key_metrics.alt_rank}
							</div>
							<div>
								<strong>Social Dominance:</strong>{' '}
								{response.key_metrics.social_dominance}
							</div>
							<div>
								<strong>Market Cap:</strong> {response.key_metrics.market_cap}
							</div>
							<div>
								<strong>24h Volume:</strong> {response.key_metrics.volume_24h}
							</div>
							<div>
								<strong>Mentions:</strong> {response.key_metrics.mentions}
							</div>
							<div>
								<strong>Engagements:</strong> {response.key_metrics.engagements}
							</div>
							<div>
								<strong>Creators:</strong> {response.key_metrics.creators}
							</div>
						</div>
					</div>
			...
			<div
				...
				<br />• <strong>Smart Voice Recognition:</strong> Auto-correction for
				crypto names
				<br />• <strong>Edit Mode:</strong> Fix transcript before submission
				<br />• <strong>Manual Input:</strong> Type queries for any
				cryptocurrency
				<br />• <strong>Query History:</strong> Recent queries saved and
				reusable
				<br />• <strong>Retry Logic:</strong> Auto-retry failed requests (3
				attempts)
				...
				<br />• <strong>Audio Controls:</strong> Play, pause, resume, stop from
				any position
				<br />
				<br />
				<strong>🚀 Production Ready:</strong>
				<br />• Rate limiting protection • Error categorization • Request
				timeouts
